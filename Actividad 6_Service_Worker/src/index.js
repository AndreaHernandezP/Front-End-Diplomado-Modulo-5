...
if('serviceWorker' in navigator){ //Controlar service worker cuando se vaya a registrar por primera vez
    window.addEventListener('load', ()=>{ //Cuando esté cargada la página
        navigator.serviceWorker.register('./service-worker.js').then(registration =>{
            console.log("SW registrado", registration);
        }).catch(err=>{
            console.log("SW no registrado", err);
        }); //Callback. Como es una promesa y JS es síncrono, se utiliza el then para asegurar que lo que sigue es lo que queremos. 
    });
}
